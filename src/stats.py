...
def get_network(network_index: int, multi_edge=True, edge_attr='datetime'):
    edgelist = (
        pd.read_pickle(f'/data/s1620444/{network_index:02}/edgelist.pkl')
        .query("source != target")
    )
    if multi_edge:
...
def single(network: int,
    ...
    edgelist_file = f'/data/s1620444/{network:02}/edgelist.pkl'
    ...
    assert os.path.isfile(edgelist_file), f'{edgelist_file} is missing'
    ...
    if np.all([os.path.isfile(os.path.join(out_directory, f)) for f in files]):
        ...
        print(
            json.dumps(
                {f: os.path.isfile(os.path.join(out_directory, f))
                 for f in files}
            )
        )
    ...
    try:
        ...
        edgelist = (
            pd.read_pickle(f'/data/s1620444/{network:02}/edgelist.pkl')
    ...
    if average_clustering:
        average_clustering_file = os.path.join(
            out_directory, 'average_clustering.float')
    ...
    if diameter and not os.path.isfile(diameter_file):
        ...
        if not os.path.isfile(temp_file):
            nx.write_edgelist(simple_graph, os.path.join(
                directory, 'edgelist.edges'), data=False)
        dist_distri_file = os.path.join(directory, 'dist_distri.npy')
        if not os.path.isfile(dist_distri_file):
            cp = subprocess.run(['./temp/teexgraph'],
                                input=f'load_undirected {temp_file}\ndist_distri',
                                encoding='ascii',
                                stdout=subprocess.PIPE)
            dist_distri = np.array(
                cp.stdout.split()).reshape(-1, 2).astype(int)
...
@app.command()
def discrete(average_clustering: bool = True,
             diameter: bool = True,
             shuffle: bool = True,
             seed: int = 42,
             verbose: bool = False):
    """"Get all discrete network stats """
    networks = [18, 20, 21, 9, 4, 8, 24, 16, 11, 10]
